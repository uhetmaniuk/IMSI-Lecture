...
main(int argc, char* argv[])
{
  Kokkos::initialize(argc, argv);
  {
...

    {
      ...
      Kokkos::View<int>                                       count("Count");
      Kokkos::parallel_for(
          "OpenMP5", policy, KOKKOS_LAMBDA(const team_member& thread) {
            Kokkos::parallel_for(
                Kokkos::TeamThreadRange(thread, 3),
                KOKKOS_LAMBDA(const int& i) { Kokkos::atomic_fetch_add(&count(), 1); });
          });
      printf(
...

    {
      ...
      Kokkos::View<int>                                       count("Count");
      Kokkos::parallel_for(
          "OpenMP6", policy, KOKKOS_LAMBDA(const team_member& thread) {
            Kokkos::parallel_for(
...

    {
      ...
      Kokkos::View<int>                                       count("Count");
      Kokkos::parallel_for(
          "OpenMP7", policy, KOKKOS_LAMBDA(const team_member& thread) {
            Kokkos::parallel_for(
                Kokkos::TeamThreadRange(thread, 5),
                KOKKOS_LAMBDA(const int& i) { Kokkos::atomic_fetch_add(&count(), 1); });
          });
