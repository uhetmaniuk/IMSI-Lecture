...
************************************************************************
************************************************************************
*****  ZBLKSLV4 ... block triangular solutions (complex)           *****
************************************************************************
************************************************************************
*
*   --------
...
*     this subroutine performs the triangular solution for 4 vectors.
...
*
*     R1 - R4   (input) COMPLEX*16 array, dimension N
*               The right hand side vectors.
*
*     S1 - S4   (output) COMPLEX*16 array, dimension N
*               The solution vectors.
*
*     T1 - T4   (temporary) COMPLEX*16 array, dimension N
*               Temporary work arrays.
*
************************************************************************
*
      SUBROUTINE  ZBLKSLV4 ( NPANEL, XPANEL, XLINDX, LINDX , XLNZ  ,
     &                      LNZ   , DEFBLK, NDEF  , LBDEF , DEF   ,
     &                      IPROW , IPCOL , PERM  , INVP  , R1, R2,
     &                      R3, R4, S1, S2, S3, S4, T1, T2, T3, T4  )
*
************************************************************************
*
*       --------------------
...
*
     ...
     &              NJ, 1, ONE, LNZ(JLPNT), JLEN, T2(FJ), NJ)
            CALL  ZTRSM ('LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',
     &              NJ, 1, ONE, LNZ(JLPNT), JLEN, T3(FJ), NJ)
            CALL  ZTRSM ('LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',
     &              NJ, 1, ONE, LNZ(JLPNT), JLEN, T4(FJ), NJ)
*
* ZGEMV - Matrix-Vector
* ZGEMM - Matrix-Matrix
*
     ...
     &             LNZ(JLPNT+NJ), JLEN, T2(FJ), 1, ZERO, S2, 1)
            CALL  ZGEMV ('NO TRANSPOSE', JLEN-NJ, NJ, -ONE, 
     &             LNZ(JLPNT+NJ), JLEN, T3(FJ), 1, ZERO, S3, 1)
            CALL  ZGEMV ('NO TRANSPOSE', JLEN-NJ, NJ, -ONE, 
     &             LNZ(JLPNT+NJ), JLEN, T4(FJ), 1, ZERO, S4, 1)
*
            JJ = JXPNT + NJ - 1
            DO  J = 1, JLEN-NJ
                JJ         = JJ + 1
                ...
                S2(J)    = ZERO
                T3(ISUB) = T3(ISUB) + S3(J)
                S3(J)    = ZERO
                T4(ISUB) = T4(ISUB) + S4(J)
                S4(J)    = ZERO
            END DO
*
        END DO
*
     ...
     &                    IPROW, IPCOL, T2(FJ), NJ, INFO )
            CALL  ZGERS ( NJ, 1, LNZ(JLPNT), JLEN, LBDEF,
     &                    IPROW, IPCOL, T3(FJ), NJ, INFO )
            CALL  ZGERS ( NJ, 1, LNZ(JLPNT), JLEN, LBDEF,
     &                    IPROW, IPCOL, T4(FJ), NJ, INFO )
        END IF
*
        DO  JJ = 1, NDEF-LBDEF
            J       = DEF(JJ)
...
*
     ...
     &                T2(FJ), 1)
                CALL  ZGEMV ( 'TRANSPOSE',JLEN-NJ, NJ,-ONE,
     &                LNZ(JLPNT+NJ), JLEN, S3, 1, ONE,
     &                T3(FJ), 1)
                CALL  ZGEMV ( 'TRANSPOSE',JLEN-NJ, NJ,-ONE,
     &                LNZ(JLPNT+NJ), JLEN, S4, 1, ONE,
     &                T4(FJ), 1)
            END IF
*
