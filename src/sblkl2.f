...
*   Modified:   08-13-2001 (Kendall H. Pierson)
*
...
************************************************************************
************************************************************************
*****  SBLKL2 ... single precision block general sparse Cholesky   *****
*****             factorization                                    *****
************************************************************************
************************************************************************
*
...
*
*     LNZ       (input/output) single precision array, dimension
*                   XLNZ(N+1)-1
...
*
*     TEMP      (temporary) single precision array, dimension TMPSIZ
...
*
*     RWORK     (temporary) single precision array, dimension RWSIZE
...
************************************************************************
*
      SUBROUTINE SBLKL2   ( NPANEL, XPANEL, PNODE , XLINDX, LINDX ,
...
*       ------------------------
        EXTERNAL           SASSMB , SCOPY , SGECP , SGEMM , SPOTRF_LDL,
     &                      SSCAL , STRSM , IGATHR, LDINDX, MMPYI
*
************************************************************************
*
        IFLAG = 0
...
*                   -------------------------------------------
                    ...
                    IWPTR = 1
                    DO  K = 1, NK
                        CALL  SCOPY ( NJ, LNZ(ILPTR), 1,
     &                                RWORK(IWPTR), 1 )
                        CALL  SSCAL ( NJ, LNZ(IDPTR), RWORK(IWPTR), 1 )
                        IDPTR = IDPTR + KPLEN + 1
                        ILPTR = ILPTR + KPLEN
                        IWPTR = IWPTR + NJ
                    END DO
...
*                       (with one column).
*                       ----------------------------------------
                        CALL  SCOPY ( NUPS, LNZ(KLPNT), 1,
     &                                RWORK, 1 )
                        CALL  SSCAL ( NUPS, LNZ(XLNZ(FK)),
     &                                RWORK, 1 )
                        CALL  MMPYI (
...
*
                            ...
                            IWPTR = 1
                            DO  K = 1, NK
                                CALL  SCOPY ( NUPS, LNZ(ILPTR), 1,
     &                                        RWORK(IWPTR), 1 )
                                CALL  SSCAL ( NUPS, LNZ(IDPTR),
     &                                        RWORK(IWPTR), 1 )
                                IDPTR = IDPTR + KPLEN + 1
                                ILPTR = ILPTR + KPLEN
                                IWPTR = IWPTR + NUPS
...
*               pivoting.
*               --------------------------------------------------
                CALL  SGECP ( NJ, LNZ(JLPNT), JLEN, TOL, ASDEF,
     &                        LBDEF,
     &                        IPROW, IPCOL, INFO )
                IF  ( INFO .NE. 0 )  THEN
                    IFLAG = 101
...
*               block.
*               ---------------------------------------------
                CALL  SPOTRF_LDL ( 'LOWER', NJ, LNZ(JLPNT), JLEN,
...
*               ---------------
     ...
     &              )
                ...
                JLPTR = JLPNT + NJ
                DO  J = FJ, LJ
                    CALL  SSCAL ( JLEN-NJ, ONE/LNZ(JDPTR),
     &                          LNZ(JLPTR), 1 )
                    JDPTR = JDPTR + JLEN + 1
                    JLPTR = JLPTR + JLEN
                END DO
