...
*  =======
*
*  SPOTF2_LDL computes the Cholesky factorization of a real symmetric
...
*
                CALL  SCOPY ( J-1, A(1,J), 1, WORK, 1 )
                DO  I = 1, J-1
                    WORK(I) = WORK(I)*A(I,I)
                END DO
                AJJ = A(J,J) - SDOT( J-1, A(1,J), 1, WORK, 1 )
...
*
                CALL  SCOPY ( J-1, A(J,1), LDA, WORK, 1 )
                DO  I = 1, J-1
                    WORK(I) = WORK(I) * A(I,I)
                END DO
                AJJ = A(J,J) - SDOT( J-1, A(J,1), LDA, WORK, 1 )
                IF  ( ABS(AJJ) .GT. TOL )  THEN
                    A(J,J) = AJJ
*
...
*
                    IF  ( J .LT. N )  THEN
                        CALL  SGEMV ( 'No transpose', N-J, J-1,
