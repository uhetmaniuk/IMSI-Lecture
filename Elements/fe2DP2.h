#ifndef FECODE_FE_2DP2_H
#define FECODE_FE_2DP2_H
...
namespace FECode {
  ...
  //! A class for two-dimensional P2 finite element.
  ...
  ///    1 -- 2
  ///
  class fe2DP2: public virtual LagrangeElement {
  ...
  private:
    ...
    // The length of d_lQuad must be two times the length of d_wQuad.
    ...
    fe2DP2(const fe2DP2 &ref);
    fe2DP2 & operator[](const fe2DP2 &ref);
  ...
  public:
    ...
    { 
      if (!d_hasPointInt)
        makePointInt();
      vv = std::vector<double>(&d_PointInt[2*iQ], &d_PointInt[2*iQ] + 2); 
    };
